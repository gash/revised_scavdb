...
function build_item_html($data, $section=FALSE) {
	$data['statuses'] = "<div class='status_column'>";
	$data['status_code'] = items::get_status_code($data['status']);
	...
	foreach ($statuses as $s=>$status) {
		...
		$data['statuses'] .= "<label class='status'><input type='radio' name='status' $v /><img src='img/status-$c.png' /> $status </label>";
		if (($i == 13) || ($i == 25)) {
		   $data['statuses'] .= "</div><div class='status_column'>";
		}
	}
	$data['statuses'] .= "</div><p class='clearing'></p>";
	$data['update_status'] = "<div class='status_column'>";
//	unset($statuses['Z']);
//	unset($statuses['x']);
//	unset($statuses['n']);
	$i = 1;
	foreach ($statuses as $s=>$status) {
		...
		$c = items::get_status_code($s);
		$data['update_status'] .= "<label class='status'><input type='radio' name='change_status' $v /><img src='img/status-$c.png' /> $status</label>";
		if (($i == 13) || ($i == 25)) {
		   $data['update_status'] .= "</div><div class='status_column'>";
		}

	}
	$data['update_status'] .= "</div><p class='clearing'></p>";
	if ($data[SNAF_SECTION] == 'edit') {
		...
		foreach ($data['files'] as $file) {
			...
			if(preg_match('/item2006/',$file)) {
				$file_image = "<p><img src='/files/{$data['item_id']}/$file' /><p>";
			}			
		}
		$data['files'] = $files;
...
//		if(strlen($file_image)) {
				$data['description'] = text2markup($data['description']) . $file_image;	
//		} else {
