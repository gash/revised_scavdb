...
class items{
    ...
    function add($data){
        ...
        $data['mtime'] = time();
	if ((empty($data['status'])) || (!(isset($data['status'])))) { $data['status'] = 'Unclaimed'; }
...
//		 }
     ...
     function get_statuses(){
         return array(
	 	'Unclaimed' => 'Unclaimed',
		'Unclaimed (Needs Research)' => 'Unclaimed (Needs Research)',
		'Unclaimed (Has Been Researched)' => 'Unclaimed (Has Been Researched)',
		'In Progress' => 'In Progress',
		'In Progress (Needs More People)' => 'In Progress (Needs More People)',
		'In Progress (Needs Supplies)' => 'In Progress (Needs Supplies)',

		'Event (Unclaimed)' => 'Event (Unclaimed)',
		'Event (Claimed)' => 'Event (Claimed)',
		'Event (Completed)' => 'Event (Completed)',
		'Event (Needs More People)' => 'Event (Needs More People)',
		'Event (Missed It)' => 'Event (Missed It)',
		'Event (Needs Supplies)' => 'Event (Needs Supplies)',
		'Partially Completed (In Box)' => 'Partially Completed (In Box)',
		'Partially Completed (Oversized)' => 'Partially Completed (Oversized)',
		'Partially Completed (Uploaded)' => 'Partially Completed (Uploaded)',
		'Partially Completed (Showcase)' => 'Partially Completed (Showcase)',
		'Partially Completed (Performance)' => 'Partially Completed (Performance)',
		'Partially Completed (Already Judged)' => 'Partially Completed (Already Judged)',
		'Completed (In Box)' => 'Completed (In Box)',
		'Completed (Oversized)' => 'Completed (Oversized)',
		'Completed (Uploaded)' => 'Completed (Uploaded)',
		'Completed (Showcase)' => 'Completed (Showcase)',
		'Completed (Performance)' => 'Completed (Performance)',
		'Completed (Already Judged)' => 'Completed (Already Judged)',
		'Road Trip' => 'Road Trip',
		'Scav Olympics (Unclaimed)' => 'Scav Olympics (Unclaimed)',
		'Scav Olympics (Claimed)' => 'Scav Olympics (Claimed)',
		'Scav Olympics (Needs More People)' => 'Scav Olympics (Needs More People)',
		'Scav Olympics (Needs Supplies)' => 'Scav Olympics (Needs Supplies)',
		'Cancelled by Judges' => 'Cancelled by Judges'
...
//				 'x' => 'Impossible',
      ...
      * Get status code
      *
      * @param $status [string]  Item status
      * @return [string]  Code
      */

     function get_status_code($status) {
	   if (preg_match('/Road Trip/', $status)) {
	   return 'r';
	   }
	   elseif ((preg_match('/Missed It/', $status)) || (preg_match('/Cancelled/', $status))) {
	   return 'x';
	   }
	   elseif (preg_match('/Event/', $status)) {
	   return 'e';
	   }
	   elseif (preg_match('/Partial/', $status)) {
	   return 'p';
	   }
	   elseif (preg_match('/Completed/', $status)) {
	   return 'Z';
	   }
	   elseif (preg_match('/Olympics/', $status)) {
	   return 'o';
	   }
     	   elseif (preg_match('/Unclaimed/', $status)) {
	   return 'n';
	   }
	   else { return 'I'; }
     }
     ...
     function get_match($match,$sort='item_id',$start=0,$num=1000){
		 ...
		 foreach($match as $k=>$v) {
		   if ($k == 'tag') {
	         ...
	         } elseif ($k == 'status_match') {
		     $cond[] = 'i.status LIKE "%' . $v . '%"';
		   } else {
		     $cond[] = "`$k`='$v'";
		   }
		 }
